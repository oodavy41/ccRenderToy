...
void main(void)
{
    ...
    vec3 cubecolor=textureCube(tex,ref).xyz*max(1.0-cuberef,0.0)+vec3(min(cuberef,1.0));


    vec3 specular=vec3(1.0,1.0,1.0)*metalless+albedo*(1.0-metalless);
    gl_FragColor = vec4(cubecolor,1.0)*lcolor*(vec4(lightColor*bright,1.0)+vec4(hlight*lightColor*specular,1.0));
}
