...
export class BasePhoneShadowMat extends AMaterial {
  ...
  constructor(GL: WebGLRenderingContext, res: ResManager) {
    ...
    super(path + "base_phone_shadow.vert", path + "base_phone_shadow.frag", GL, res);
    this.metalless = 0;
    this.smoothness = 0;
  }
  ...
  draw() {
    ...
    this.setUniformI1i("shadowMap", this.scene.lights["Main"].depthFrame, this.scene.GL, 0);
    this.setUniformM4f("lightMVP", this.scene.lights["Main"].lightMVP, this.scene.GL);
